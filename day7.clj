...
(defn calculate [tree k]
  (let [v (tree k)
    ...
    (if (contains? tree kw)
      ...
      ; if it isn't, check if it's an expression
      (if-let [[all l c r] (re-find #"(.+) (AND|OR|RSHIFT|LSHIFT) (.+)" v)]
        (execute tree c l r)
        (if-let [[all c l] (re-find #"(NOT) (.+)" v)]
          (execute tree c l 0)
          ; otherwise, spit out what it is as an integer
          (Integer/parseInt v))))))

(defn execute [tree command lhs rhs]
  (let [l (if (integer? lhs) lhs (if (numeric? lhs) (Integer/parseInt lhs) (calc-memoized tree (keyword lhs))))
        r (if (integer? rhs) rhs (if (numeric? rhs) (Integer/parseInt rhs) (calc-memoized tree (keyword rhs))))]
    (case command
      "AND" (bit-and l r)
      "OR" (bit-or l r)
      "RSHIFT" (bit-shift-right l r)
      "LSHIFT" (bit-shift-left l r)
      "NOT" (bit-not l))))

;; memoize the function for faster lookups
(def calc-memoized (memoize calculate))
