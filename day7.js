...
var parseInstruction = function(instruction, values) {
	var i = instruction.split(" -> ");
  var lhs = i[0];
  var rhs = i[1];
  if (lhs.indexOf("AND") !== -1) {
    var v1 = lhs.split(" AND ")[0];
    var v2 = lhs.split(" AND ")[1];
    if (values.hasOwnProperty(v1)) v1 = values[v1];
    else v1 = parseInt(v1);
    if (values.hasOwnProperty(v2)) v2 = values[v2];
    else v2 = parseInt(v2);
    lhs = v1 & v2;
  }
  else if (lhs.indexOf("OR") !== -1) {
    var v1 = lhs.split(" OR ")[0];
    var v2 = lhs.split(" OR ")[1];
    if (values.hasOwnProperty(v1)) v1 = values[v1];
    else v1 = parseInt(v1);
    if (values.hasOwnProperty(v2)) v2 = values[v2];
    else v2 = parseInt(v2);
    ...
    var v1 = lhs.split(" LSHIFT ")[0];
    var v2 = parseInt(lhs.split(" LSHIFT ")[1]);
    if (values.hasOwnProperty(v1)) v1 = values[v1];
    else v1 = parseInt(v1);
    lhs = v1 << v2;
  }
  else if (lhs.indexOf("RSHIFT") !== -1) {
    var v1 = lhs.split(" RSHIFT ")[0];
    var v2 = parseInt(lhs.split(" RSHIFT ")[1]);
    if (values.hasOwnProperty(v1)) v1 = values[v1];
    else v1 = parseInt(v1);
    lhs = v1 >> v2;
  }
  else if (lhs.indexOf("NOT") !== -1) {
    var v1 = lhs.split("NOT ")[0];
    if (values.hasOwnProperty(v1)) v1 = values[v1];
    else v1 = parseInt(v1);
    lhs = ~v1;
  }
...
for (var i = 0; i < instructions.length; i++) {
  values = parseInstruction(instructions[i], values);
}

console.log(values);
console.log("Part 1:", values["a"]);
